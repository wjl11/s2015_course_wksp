...
% part 2a 1 (EDITED)
switch trial
    case 1 % simulate R = 0.026
        z1=0.95;
        z2=1;
        R = (z2-z1)/(z2+z1);
    case 2 % simulate R = 0.5
        z1=1/3; 
        z2=1;
        R = (z2-z1)/(z2+z1);
end
...
figure(1)
subplot(1,2,trial)
imagesc(lats*1e3,deps*1e3,dbzero(abs(hilbert(bm1'))),[-45 0]), colormap gray, 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% RERUN WITH DISPLACED SCATTERER %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Generate field maps %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c(round(nY/2)-1:round(nY/2)+1,foc-1)=z2*c0; 
c(round(nY/2)-1:round(nY/2)+1,foc)=z1*c0;
%%% Launch %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
launchTotalFullWave2(c0,omega0,wY,wZ,duration,p0,ppw,cfl,c',rho',A',bovera',incoords,outcoords,icmat);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
tic
...
genout = readGenoutSlice(['genout.dat'],0:nRun-1,size(outcoords,1));
idc=find(outcoords(:,3)==1);
p3 = reshape(genout(:,idc),size(genout,1),size(modidy,2),size(modidy,1));
% imagesc(squeeze(p3(end,:,:))), colorbar
%for i=1:size(p3,1)
%  imagesc(squeeze(p3(i,:,:))', [-1 1]*p0), title(num2str(i)), drawnow
%end

idc=find(outcoords(:,3)==2);
pxducer3=genout(:,idc);
pxducer3=pxducer3(:,round(size(pxducer3,2)/3*2+1:size(pxducer3,2)));
...
for ii=1:length(lats)
  for jj=1:length(deps)
    bm3(ii,jj)=sum(pxducer3(idps{ii,jj}));
  end
end
toc
...
figure(2)
subplot(1,2,trial)
imagesc(lats*1e3,deps*1e3,dbzero(abs(hilbert(bm3'))),[-45 0]), colormap gray
xlabel('y (mm)'), ylabel('z (mm)')
axis equal, axis tight
title(sprintf('R = %.2f',R))
drawnow
